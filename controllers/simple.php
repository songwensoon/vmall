...
class Simple extends IController
{
	...
	function new_email_reg_four()
	{
		$user_id  = IFilter::act(IReq::get('uid','get'));
		$rand = IFilter::act(IReq::get('k','get'));
		$time = IFilter::act(IReq::get('u','get'));
		if(!$user_id){
		...
		$activedataObj = new IModel('active_data');
		$where   = 'uid = '.$user_id;
		$activedataRow = $activedataObj->getObj($where);
		...
		if($time==$activedataRow['time']&&$activedataRow['rand']==$rand&&$userRow['email_verify']!=1){
			$dataArray = array(
			'email_verify'=>1	
			);
		...
		elseif($userRow['email_verify']!=1&&$activedataRow['rand']&&$rand ){
			$this->error = "连接失效，请重新发激活邮件";
		}
		elseif($userRow['email_verify']!=1){
			$this->redirect('new_email_reg_two');die;
		}
		else{
			header("Location:index.php");die;
		}

		$this->redirect('new_email_reg_four',false);
	}
	function new_email_reg_three()
	{
		$user_id  = IFilter::act(ISafe::get('user_id'),'int');
		if(!$user_id){
			header("Location: index.php");die;
		...
		$where   = 'id = '.$user_id;
		$userRow = $userObj->getObj($where);
		if($userRow['email_verify']==1){
			$this->redirect('new_email_reg_four');
		}
		if(!$userRow['email']){
			$this->redirect('new_email_reg_two');
		}
		$this->email = $userRow['email'];
		$this->redirect('new_email_reg_three',false);
	}
	function resendemail()
	{
		$user_id  = IFilter::act(ISafe::get('user_id'),'int');
		$email = IFilter::act(IReq::get('remail','post'));
		if(isset($email))
		{	
			$siteConfigObj = new Config("site_config");
			$site_config   = $siteConfigObj->getInfo();
			$weburl = $site_config['url'];
			$weburl_name = $site_config['name'];
			$weburl_email = $site_config['email'];
			$weburl_desc = $site_config['index_seo_description'];
			$member_name = $username;
			$weburl_tel = $site_config['phone'];

			$rand=create_password("8");
			$time=md5(create_password("5"));
			$link=$weburl."/index.php?controller=simple&action=new_email_reg_four&uid=$user_id&u=$time&k=$rand";
			
			$activedataObj = new IModel('active_data');
			
			$where   = 'uid = '.$user_id;
			$activedataRow = $activedataObj->getObj($where);
			$activedata_array = array(
				'time'=>$time,
				'rand'=>$rand
			);
			if($activedataRow){
				$activedataObj->setData($activedata_array);
				$activedataObj->update($where);
			}else{
				$activedata_array['uid'] = $user_id;
				$activedataObj->setData($activedata_array);
				$activedataObj->add();
			}

			$msgObj = new IModel('msg_template');
			$where   = 'id = 4';
			$mail_temp = $msgObj->getObj($where);
			$con=$mail_temp['content'];
			$title=$mail_temp['title'];
			$user=$username;
			$time=date("Y-m-d H:i:s");
			$logo='<img height="24" src="'.$weburl.'/image/logo.png"  style="border:none;margin:0;">';
		
			$menu = "";
			$ar1=array('{$menu}','{$time}','{$logo}','{$member_name}','{$weburl_name}','{$link}','{$weburl_email}','{$weburl_tel}','{$weburl_url}','{$weburl_desc}','{$weburl_desc}');
			$ar2=array($menu,$time,$logo,$member_name,$weburl_name,$link,$weburl_email,$weburl_tel,$weburl_url,$weburl_desc,$weburl_desc);
			$con=str_replace($ar1,$ar2,$con);
		
			$ar3=array('{$member_name}','{$weburl_name}');
			$ar4=array($user,$weburl_name);
			...
			$status  = $smtp->send($email,$title,$con);
			if(!$status){
				echo "false";
				die;
			}
	...
	function new_email_reg_two()
	{
		...
		if($email)
		{
			$username  = IFilter::act(ISafe::get('username'),'string');
			$where   = 'email = "'.$email.'"';
			$userRow = $userObj->getObj($where);
			
			if(!empty($userRow)){
				$this->redirect('new_email_reg_two',false);
			}
			else{
				...
				$this->redirect('new_email_reg_three');
	...
	public function checkmemberno()
	{
		$id      = IFilter::act(IReq::get('admin_no','get'));
		$userObj = new IModel('admin');
		$where   = 'id ='.$id;
		$userRow = $userObj->getObj($where);
		if(!empty($userRow))
		{
    ...
    function reg_act()
    {
    	$admin_no      = IFilter::act(IReq::get('admin_no','post'));
		...
		$realname	= IFilter::act(IReq::get('realname','post'));
		$tel		= IFilter::act(IReq::get('tel','post'));
		$certificatenum = IFilter::act(IReq::get('certificatenum','post'));
    	...
    	if($message == '')
    	{
    		...
    		if($user_id)
    		{
				...
				$site_config   = $siteConfigObj->getInfo();
				$weburl = $site_config['url'];
				header("Location: ".$weburl."/index.php?controller=simple&action=new_email_reg_two");
				exit;
				//自定义跳转页面
